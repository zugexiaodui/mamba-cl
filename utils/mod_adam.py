...
class ModAdam(Optimizer):
    def __init__(self,
                 params: ParamsT,
                 update_projection_dict: dict[int, Tensor],
        ...
        self.update_projection_dict = update_projection_dict
        ...
        if fused:
            ...
            # (crcrpar): [low prec params & their higher prec copy]
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            adam(
                ...
                update_projection_dict=self.update_projection_dict,
...
ModAdam.__doc__ = r"""Implements Adam algorithm.
...
def adam(params: List[Tensor],
    ...
    if func.__name__ not in (_l := ('_multi_tensor_adam',)):
        raise NotImplementedError(f"Only support for {_l}")
    ...
    func(params,
         ...
         found_inf=found_inf,
         update_projection_dict=update_projection_dict,
         arg_dict=arg_dict)
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    for ((
        ...
        if capturable:
            ...
            updates = [torch.zeros_like(_p) for _p in device_params]
            torch._foreach_addcdiv_(updates, device_exp_avgs, exp_avg_sq_sqrt, step_size)

            _l2 = len(list(filter(lambda _pid: _pid in update_projection_dict.keys(), map(id, device_params))))
            if _l2 == 0:
                torch._foreach_add_(device_params, updates)
            else:
                for _up, _dp in zip(updates, device_params):
                    if (_pid := id(_dp)) in update_projection_dict:
                        _src_shape, _src_data_ptr = _up.shape, _up.data_ptr()

                        if 'interm_reader_step' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_step']
                            _up.data = torch.matmul(_up, _proj).data
                        elif 'interm_reader_B' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_B']
                            _up.data = torch.matmul(_up, _proj).data
                        elif 'interm_reader_C' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_C']
                            _up.data = torch.matmul(_up, _proj).data
                        elif 'interm_reader_A' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_A']
                            _up.data = torch.matmul(_proj, _up).data
                        elif 'interm_reader_out' in update_projection_dict[_pid]:
                            _proj = update_projection_dict[_pid]['interm_reader_out']
                            _up.data = torch.matmul(_up, _proj).data
                        else:
                            raise ValueError()

                        assert _src_shape == _up.shape and _up.data_ptr() != _src_data_ptr, f"{_src_shape} ~ {_up.shape}"
